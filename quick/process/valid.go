...
// FilterDate 过滤数据字段
func (p *Process) FilterDate(data ztype.Map, fields []string) ztype.Map {
	l := len(fields)
	if l == 0 {
		return data
	}
...
// VerifiData 验证数据
func (p *Process) VerifiData(data ztype.Map, active activeType) (ztype.Map, error) {
	d := make(ztype.Map, len(p.Define.Fields))
	for name, column := range p.Define.Fields {
		...
		if ok {
			...
			case schema.Time:
				...
				case define.DataTime:
					d[name] = t
				case time.Time:
					d[name] = define.DataTime{Time: t}
				case int:
					d[name] = define.DataTime{Time: ztime.Unix(ztype.ToInt64(v))}
				case string:
					r, err := ztime.Parse(t)
					if err != nil {
						return d, errors.New(label + ": 时间格式错误")
					}
					d[name] = define.DataTime{Time: r}
				}
			case schema.JSON:
